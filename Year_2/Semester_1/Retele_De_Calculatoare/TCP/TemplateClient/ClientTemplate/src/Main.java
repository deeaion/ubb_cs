...
public class Main {
    ...
    public static void main(String args[])
    {
        ...
        try{
            ...
            writeIntegertoSocket(sir.length(),socketOut);
            writeStringToSocket(sir,socketOut);
            readResult(socketIn);
    ...
    private static void readResult(DataInputStream socketIn) throws IOException {
        char c=(char)socketIn.readUnsignedByte();
        int numar=0;
        numar=socketIn.readUnsignedShort();
        byte [] pozitii=new byte[numar];
        socketIn.read(pozitii);
        System.out.println("The Least found character is "+c);
        System.out.println("Pozitii: ");
        for(int i=0;i<pozitii.length;i++)
        {
            System.out.print(pozitii[i]+" ");
        }
    }

    private static byte[] transformStringNumbersToByte(String sir)
    {
        String [] numberStrings=sir.split(" ");
        byte [] numbers=new byte[numberStrings.length];
        for(int nr=0;nr<numberStrings.length;nr++)
        {
            try
            {
                numbers[nr]=Byte.parseByte(numberStrings[nr]);
            }
    ...
    /**WRITE TO SERVER*/
    private static void writeStringToSocket(String sir1, DataOutputStream socketOut) throws IOException {
    ...
    private static void writeIntegertoSocket(int number,DataOutputStream socketOut) throws IOException {
        socketOut.writeShort(number);
        socketOut.flush();
    }
    private static void writeByteArrayToSocket(byte [] toSend,DataOutputStream socketOut) throws IOException {
