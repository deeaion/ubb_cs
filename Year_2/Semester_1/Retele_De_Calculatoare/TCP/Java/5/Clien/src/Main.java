...
public class Main {
    ...
    public static void main(String args[])
    {
        ...
        try{
            ...
            byte [] divizori=readResult(socket,soIn);
            System.out.println("Divizorii "+number+"sunt: ");
            for(int i=0;i<divizori.length;i++)
            {
                System.out.print(divizori[i]+" ");
            }
    ...
    private static byte [] readResult(Socket socket,DataInputStream soIn) throws IOException {
        byte [] divizori=new byte[0];
        int nrDivizori=0;
        try
        {
            nrDivizori=soIn.readUnsignedShort();
            divizori=new byte[nrDivizori];
            soIn.read(divizori);
