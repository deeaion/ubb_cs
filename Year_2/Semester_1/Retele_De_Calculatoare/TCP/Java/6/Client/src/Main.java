...
public class Main {
    ...
    public static void main(String args[])
    {
        ...
        try{
            ...
            String sirCharacters=readStringFromConsole("Dati sir:",reader);
            char Character=readCharConsole("Dati caracter:",reader);
            //trimit lenght first
            writeIntegertoSocket(sirCharacters.length(),socket,socketOut);
            //trimit sirul
            writeStringToSocket(sirCharacters,socket,socketOut);
            writeCharToSocket(Character,socketOut);
            //primire rezultat
            int [] pozitii_found=readResult(socket,socketIn);
            for(int poz:pozitii_found)
            {
    ...
    private static int [] readResult(Socket socket,DataInputStream socketIn) throws IOException {
        //I NEED TO DECLARE FIRST -> it can be chars or numbers
        int number=0;
        number=socketIn.readInt();
        System.out.println(number);
        byte[] pozitii=new byte[number];
        socketIn.read(pozitii); //-> read all byte
        int [] pozInt=new int[number];
        for(int i=0;i<number;i++)
        {
               pozInt[i]=pozitii[i]+1;
