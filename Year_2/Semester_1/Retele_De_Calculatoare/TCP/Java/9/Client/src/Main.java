...
public class Main {
    ...
    public static void main(String args[])
    {
        ...
        try{
            ...
            String sirNumere=readStringFromConsole("Dati primul sir:",reader);
            String sirNumere2=readStringFromConsole("Dati al doilea sir: ",reader);
            byte [] numere1=transformStringNumbersToByte(sirNumere);
            byte [] numere2=transformStringNumbersToByte(sirNumere2);
            writeIntegertoSocket(numere1.length,socket,socketOut);
            writeIntegertoSocket(numere2.length,socket,socketOut);
            writeByteArrayToSocket(numere1,socketOut);
            writeByteArrayToSocket(numere2,socketOut);
    ...
    private static void readResult(Socket socket,DataInputStream socketIn) throws IOException {
        int n=0;
        n=socketIn.readUnsignedShort();
        byte [] numere=new byte[n];
        socketIn.read(numere);
        System.out.println("Numerele care nu sunt comune: ");
        for(byte i:numere)
        {
            System.out.print(i+" ");
        }
