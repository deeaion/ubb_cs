...
errorHandle newClient(int s,struct sockaddr_in serverin,int port,struct sockaddr_in *client,int l)
{
    ...
    uint16_t size;
    uint32_t rezultat;
    char* optiune= getSir(&s,&size,client,&errorH);
    if(strcmp(optiune,"yes")==0)
    {
        rezultat=handleClient();
    }
    else
    {
        if(strcmp(optiune,"no")==0)
        {
            rezultat=0;
        }
    }
    send4ByteNumber(&s,&rezultat,client,&errorH);
