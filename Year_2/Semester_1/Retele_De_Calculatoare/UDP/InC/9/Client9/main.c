...
int main(int argc, char **argv) {
    ...
    while(1)
    {
        char *getoption= readStringFromUser("Doriti sa primiti data? Enter yes or no!\n");
        if(strcmp(getoption,"yes\n")==0|| strcmp(getoption,"no\n")==0)
        {
            printf("You entered a valid response. Congrats!\n");
            int len= strlen(getoption);
            sendStringToServer(c,getoption,&len,&server);
            break;
        }
        else
        {
    ...
    uint32_t result= get4ByteInteger(c,&server);
    if(result!=0)
    {time_t timeF=result;
    char* date=ctime(&timeF);
    printf("Today is: %s",date);}
    else {
        printf("You said no I guess so no date for you !:(\n");
    }
