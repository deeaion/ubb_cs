...
uint16_t handleClient(uint16_t nr1,uint16_t nr2)
{
    return nr1+nr2;
}
errorHandle newClient(int s,struct sockaddr_in serverin,int port,struct sockaddr_in *client,int *l)
{
    ...
    //reading the numbers
    uint16_t nr1,nr2,sum;
    nr1= getunsignedInteger(s,client,&errorH);
    if(!errorH.succes)
    {
        return errorH;
    }
    nr2= getunsignedInteger(s,client,&errorH);
    if(!errorH.succes)
    {
        return errorH;
    }
    sum=handleClient(nr1,nr2);
    //trimit
    sendNumber(s,&sum,client,&errorH);
    if(!errorH.succes)
    {
        return errorH;
    }
