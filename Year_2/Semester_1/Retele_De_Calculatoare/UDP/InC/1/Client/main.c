...
uint16_t* getNumbers(int s,int *size,struct sockaddr_in* socket,int *l)
{
    int recvResult=
            recvfrom(s,size,sizeof (size),MSG_WAITALL,
...
//read 4 byte integer
uint32_t get4ByteInteger(int c,struct sockaddr_in* server,int *l)
{
    uint32_t result;
...
char getChar(int c,struct sockaddr_in* server,int *l)
{
    char result;
    int recvFromRes= recvfrom(c,&result, sizeof(result),0,(struct sockaddr*)&server, &l);
    if(recvFromRes<0)
...
int main(int argc,char **argv) {
    ...
    nr1= readNumber("Dati primul numar: \n");
    nr2= readNumber("Dati al doilea  numar: \n");

    //trimitem numerele
    sendNumber(&c,&nr1,&server);
    sendNumber(&c,&nr2,&server);

    //raspuns
    uint16_t suma=0;
    int l= sizeof(server);
    suma=getunsignedInteger(c,&server);

    //printez rezultat
    printf("Suma numerelor %hu and %hu is %hu!\n",nr1,nr2,suma);
    close(c);
    return 0;
}
