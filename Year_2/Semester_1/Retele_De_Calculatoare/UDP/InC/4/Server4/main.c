...
void sendIntegerVectorToServer(int c,uint16_t numbers[],int*len,struct sockaddr_in *server,errorHandle *errorH)
{
    *len= htons(*len);
    int sendToResult1= sendto(c,len,sizeof ((*len)),0,(struct sockaddr* )server, sizeof((*server)));
    if(sendToResult1<0)
...
char*  handleClient(char* sirToMirror,uint16_t size)
{
    char* sirMirrored=(char*) malloc(size+1);
    int pos=0;
    for(int i=size-1;i>=0;i--)
    {
        sirMirrored[pos++]=sirToMirror[i];
    }
    return sirMirrored;
}
errorHandle newClient(int s,struct sockaddr_in serverin,int port,struct sockaddr_in *client,int l)
{
    ...
    uint16_t size;
    char *sirIn= getSir(&s,&size,client,&errorH);
    char *sirToSend=handleClient(sirIn,size);
    size=size+1;
    sendStringToServer(s,sirToSend,&size,client,&errorH);
