...
int main(int argc,char **argv) {
    ...
    char *getSir= readStringFromUser("Dati sir: \n");
    int len= strlen(getSir);
    sendStringToServer(c,getSir,&len,&server);
    //get result
    uint16_t result= getunsignedInteger(c,&server);
    printf("Sunt %hu spatii!\n",result);
    close(c);
    return 0;
}
