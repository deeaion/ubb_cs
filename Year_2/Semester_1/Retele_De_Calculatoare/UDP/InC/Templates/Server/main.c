...
uint16_t getInteger(int c,struct sockaddr_in* server,errorHandle *errorH)
{
    uint16_t result;
...
{
    *toSend= htonl(*toSend);
    int sendToRes= sendto(s,toSend, sizeof (uint32_t),0,(struct sockaddr* )client,sizeof (*client));
    if(sendToRes<0)
    {
...
void sendString(int c,char* string,int *len,struct sockaddr_in *client,errorHandle *errorH)
{
    *len=(*len);
    *len= htons(*len);
    int sendToResult1= sendto(c,len,sizeof ((*len)),0,
    ...
    *len= ntohs((*len));
    sendToResult1= sendto(c,string,*len,0,
                          (struct sockaddr*)client, sizeof(*client));
    if(sendToResult1<0)
    {
        changeError(errorH,"Could not send String!...\n");
