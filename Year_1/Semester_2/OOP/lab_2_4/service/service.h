 ...
 */
double modify_electronic_price_by_id(MyList* store,int id, double price);
/*
 * Modifying an electronic by quantity
 */
float modify_electronic_quantity_by_id(MyList* store,int id, float price);
/*
 * Deleting a product by id
 */
MyList delete_product_by_id(MyList* store, int id);
/*
 ...
 */
//for ascending order
MyList sort_in_stock_electronics_ascending_order(MyList* store, int way);
//for descending order
MyList sort_in_stock_electronics_descending__order(MyList* store, int way);
/*
 * preconditions: user must give a criteria of the filter
 * returns: a list filtered by  the criteria given
 *
 */
MyList filter_by_criteria(MyList* store, int criteria_choice);
