...
void Tests::testStoreAdd_Search() {
    DynamicVector<Subject> tests;
    SubjectsRepo repo(tests);
    Subject testsubject{"1", 1, "1", "1"};
...
void Tests::testUpdate_Remove() {
    ...
    Subject testsubject{"1", 1, "1", "1"};
    repo.store(testsubject);
    const DynamicVector<Subject>& uh= repo.getAll();
    assert(uh.getSize()==1);
...
void Tests::testAdd_SearchService() {
    ...
    const DynamicVector<Subject>& sbjs= serv.getAll();
    assert(sbjs.getSize()==1);
    try{
        serv.addSubject("1",1,"1","1");
    }
    catch (RepositoryException &mes)
...
void Tests::testSortService() {
    ...
    serv.addSubject("1",1,"1","1");
    DynamicVector<Subject> sn=serv.sortByName();
    assert(ver1.operator==(sn[1]));
    assert(ver2.operator==(sn[0]));
    DynamicVector<Subject> sh=serv.sortByHours();
    assert(ver1.operator==(sh[1]));
    assert(ver2.operator==(sh[0]));
    DynamicVector<Subject> stt=serv.sortByTeacherandType();
    assert(ver1.operator==(stt[1]));
    assert(ver2.operator==(stt[0]));
    DynamicVector<Subject> fh=serv.filterByHours(2);
    assert(ver1.operator==(fh[0]));
    DynamicVector<Subject> ft=serv.filterByTeacher("2");
    assert(ver1.operator==(ft[0]));
    serv.deleteSubject("1","1");

    DynamicVector<Subject> s=serv.sortByName();
