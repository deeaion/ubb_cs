...
public class FiniteAutomata {
    ...
    public boolean acceptsSequence(String sequence) {
        ...
        if (sequence.isEmpty() && finalStates.contains(initialState)) {
            System.out.println("Sequence accepted: ε (initial state is also a final state)");
            return true;
        }
        State currentState = initialState;
        for (char symbol : sequence.toCharArray()) {
            boolean transitionFound = false;
            System.out.println("Current state: " + currentState.getValue() + ", Symbol: " + symbol);

            for (Transition transition : transitions) {
                System.out.println("Checking transition: " + transition.getSource().getValue() + " " + transition.getAlphabet().getValue() + " " + transition.getDestination().getValue());
                System.out.println("Source match: " + transition.getSource().equals(currentState));
                System.out.println("Symbol match: " + transition.getAlphabet().getValue().equals(String.valueOf(symbol)));
            ...
            if (!transitionFound) {
                System.out.println("No transition found for symbol: " + symbol + " from state: " + currentState);
                return false;
            }
        }

        if (finalStates.contains(currentState)) {
            System.out.println("Sequence accepted, final state: " + currentState.getValue());
            return true;
        } else {
            System.out.println("Reached non-final state: " + currentState.getValue());
            return false;
        }
    }
